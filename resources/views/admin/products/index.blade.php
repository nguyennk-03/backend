    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content rounded-lg shadow-lg">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" id="gamingModalLabel"><i class="fas fa-plus-circle me-2"></i> Thêm
                    sản phẩm mới</h5>
                ...
                <form action="{{ route('san-pham.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="price" id="price" class="form-control border-0 shadow-sm"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="stock" id="stock" class="form-control border-0 shadow-sm"
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="description" id="description" class="form-control border-0 shadow-sm"
                                placeholder="Nhập mô tả sản phẩm" rows="4">{{ old('description') }}</textarea>
                            @error('description')
                            <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-12">
                            ...
                            <div class="input-group">
                                <input type="file" name="image" id="image" class="form-control border-0 shadow-sm"
                                    accept="image/*" data-preview="preview_add">
                                <button type="button" class="btn btn-outline-primary"
                                    onclick="document.getElementById('image').click()">Chọn file</button>
                            </div>
                            <div class="image-preview mt-3" id="preview_add">
                                <img id="preview_add_img" src="" alt="Ảnh xem trước" class="rounded shadow-sm d-none"
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @if (!empty($item->image))
                            <img src="{{ asset($item->image) }}" class="img-thumbnail rounded"
                                style="object-fit: cover; max-width: 100px; max-height: 100px;" alt="{{ $item->name }}">
                            @else
        ...
        <div class="modal-content rounded-lg shadow-lg">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" id="showModalLabel{{ $item->id }}"><i
                        class="fas fa-info-circle me-2"></i> Chi tiết sản phẩm
                    #{{ $item->id }}</h5>
        ...
        <div class="modal-content rounded-lg shadow-lg">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" id="editModalLabel{{ $item->id }}"><i
                        class="fas fa-edit me-2"></i> Chỉnh sửa sản phẩm
                    #{{ $item->id }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"
                    aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                ...
                <form action="{{ route('san-pham.update', $item->id) }}" method="POST"
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label for="brand_id_{{ $item->id }}"
                                class="form-label fw-semibold">Thương hiệu</label>
                        ...
                        <div class="col-md-6">
                            <label for="category_id_{{ $item->id }}"
                                class="form-label fw-semibold">Danh mục</label>
                        ...
                        <div class="col-md-12">
                            <label for="name_{{ $item->id }}"
                                class="form-label fw-semibold">Tên sản phẩm</label>
                        ...
                        <div class="col-md-6">
                            <label for="price_{{ $item->id }}"
                                class="form-label fw-semibold">Giá</label>
                        ...
                        <div class="col-md-6">
                            <label for="stock_{{ $item->id }}"
                                class="form-label fw-semibold">Số lượng tồn kho</label>
                        ...
                        <div class="col-md-12">
                            <label for="image_{{ $item->id }}"
                                class="form-label fw-semibold">Chọn hình ảnh</label>
                            <div class="input-group">
                                <input type="file" name="image" id="image_{{ $item->id }}"
                                    class="form-control border-0 shadow-sm" accept="image/*"
                                    data-preview="preview_{{ $item->id }}">
                                <button type="button" class="btn btn-outline-primary"
                                    onclick="document.getElementById('image_{{ $item->id }}').click()">Chọn
                                    file</button>
                            </div>
                            <div class="mt-3 d-flex align-items-center gap-3">
                                @if (!empty($item->image))
                                <div class="current-image">
                                    <label class="form-label small text-muted">Hình ảnh
                                        hiện tại:</label>
                                    <img src="{{ asset($item->image) }}"
                                        alt="Hình ảnh hiện tại" class="rounded shadow-sm"
                                        style="width: 60px; height: 60px; object-fit: cover;">
                                </div>
                                @endif
                                <div class="preview-image">
                                    ...
                                    <img id="preview_{{ $item->id }}" src=""
                                        alt="Ảnh xem trước" class="rounded shadow-sm d-none"
                                        style="width: 60px; height: 60px; object-fit: cover;">
                                </div>
                            </div>
                            @error('image')
                    ...
                    <div class="modal-footer border-0 pt-4">
                        <button type="button" class="btn btn-secondary btn-sm fw-semibold"
                            data-bs-dismiss="modal">Đóng</button>
                        <button type="submit" class="btn btn-primary btn-sm fw-semibold">Cập
                            nhật</button>
                    </div>
                </form>
            </div>
