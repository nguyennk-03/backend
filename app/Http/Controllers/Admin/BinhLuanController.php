...
class BinhLuanController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $comment = Comment::findOrFail($id);  // Sử dụng model Comment thay vì Review

        // Chỉ cập nhật trạng thái
        $comment->status = $request->status;
        $comment->save();  // Lưu lại thay đổi

        return redirect()->route('binh-luan.index')->with('success', 'Trạng thái bình luận đã được cập nhật!');  // Sử dụng route cho bình luận
    }
